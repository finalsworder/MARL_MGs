...
from rl_torch.cmdp_offpolicy_cnn_ref import multi_agent_system
import torch
from rl_torch.nn import Actor_sequence, Actor_sequence_sig, Critic_sequence,\
    ...
    Actor_sequence_linear, Critic_sequence_linear,\
...
ref_coeff_decay = 1
exploration_prob = .9
exploration_prob_decay = .995
exploration_prob_min = .05

g_ub = 10
# record_min = 1e4
record_interval = 10
logfile = open('results/2bus_1MG/2bus_1MG' + time.strftime('%Y_%m_%d_%H_%M_%S.csv'), 'w+', newline='')
env = DN_2bus_1MGs()
...
agents = multi_agent_system(agent_num=env.MG_num, device=device, communication_matrix=w,
                            actor_fn=Actor_sequence, sample_fn=normal_sample,
                            critic_fn=Critic_sequence_linear, state_dim=state_dim,
...
for episode in range(EPISODES):
    ...
    for _ in range(max_episode_length):
        ...
        state = state_
        sequence = sequence_
        ref_actions = env.get_ref_action()
        ...
        agents.store(state, sequence, actions, ref_actions, c, g, state_, sequence_)

        if len(agents.replay_buffer.ids) >= start_train:
            if t % actor_interval == 0:
                ...
                exploration_prob = min(exploration_prob_decay * exploration_prob, exploration_prob_min)
                agents.var = action_var
                ref_coeff *= ref_coeff_decay
            if t % critic_interval == 0:
                agents.train_batch(batch_size=batch_size, critic_iter=1, actor_iter=0, train_step=train_step,
                                   ref_coeff=ref_coeff)
                for agent in agents.agents:
                    step_size = 1e3 / (train_step + 1e3 + 1)
                    lmbd_subgradient = agent.mu_g * env.MG_num - g_ub
