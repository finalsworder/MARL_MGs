from env.IEEE_5bus_4MGs_private_c import DN_5bus_4MGs
from rl_torch.cmdp_offpolicy_cnn_private_c import multi_agent_system
import torch
...
sequence_len = 288
start_train = 10 * 288
dual_start_train = 100 * 288
train_interval = int(100)
action_var = .5
action_var_decay = .995
action_var_min = .2
g_ub = float(input('g_ub:'))
c_reg = 2e-1
private_c_reg = 10
private_c_reg_decay = 1
# record_min = 1e4
record_interval = 10
time_str = time.strftime('%Y_%m_%d_%H_%M_%S')
print(time_str)
logfile_name = 'results/5bus_4MGs/5bus_4MGs_gub%.3f_' % g_ub + time_str + '.csv'
logfile = open(logfile_name, 'w+', newline='')
cg_file_name = 'results/5bus_4MGs/5bus_4MGs_gub%.3f_' % g_ub + time_str + '_cg.csv'
cgfile = open(cg_file_name, 'w+', newline='')
cgfile.write('c,g,lmbd0,lmbd1,lmbd2,lmbd3\n')
env = DN_5bus_4MGs()
...
state_, sequence_, _, _, _ = env.step(action_all)
sequence_ = sequence_[:, :sequence_len]
...
w2 = np.array([[.9, .1, .0, .0],
               ...
               [.0, .0, .9, .1],
               [.1, .0, .0, .9]])
writer = SummaryWriter(log_dir='tensorboard_log/33bus_4MGs/' + time.strftime('%Y_%m_%d_%H_%M_%S'))
agents = multi_agent_system(agent_num=env.MG_num, device=device, communication_matrix=w2,
                            ...
                            memory_volume=buffer_size, g_ub=g_ub, lmbd_max=10000,
                            writer=writer)
...
for episode in range(EPISODES):
    ...
    for _ in range(max_episode_length):
        ...
        # actions = np.array([[1, 1, 1, 0.5] for _ in range(env.MG_num)])
        actions = env.get_ref_action()
        # for i in range(env.MG_num):
        #     actions[i][0] = 1
        #     actions[i][2] = 1
        state_, sequence_, c, private_c, g = env.step(actions)
        sequence_ = sequence_[:, :sequence_len]
        c = c * c_reg
        private_c = private_c * c_reg
        ...
        episode_c_all += c_sum
        episode_g_all += g_sum
        cgfile.write('%f, %f, %f, %f, %f, %f\n' % (c_sum, g_sum, agents.agents[0].lmbd, agents.agents[1].lmbd,
                                                   agents.agents[2].lmbd, agents.agents[3].lmbd))
        # agents.update_mu_g(g=g, step_size=1e-3 / np.log(t + 1))
        agents.update_mu_g(g=g, step_size=1e-3)
        agents.store(state, sequence, actions, c, private_c, g, state_, sequence_)
        ...
        if len(agents.replay_buffer.ids) >= start_train and t % train_interval == 0:
            agents.train_batch(batch_size=batch_size, critic_iter=1, actor_iter=1, train_step=train_step,
                               private_c_reg=private_c_reg)
            for agent in agents.agents:
                step_size = 1e4 / (train_step + 5e5)
                lmbd_subgradient = agent.mu_g * env.MG_num - g_ub
                if t >= dual_start_train:
            ...
            private_c_reg = private_c_reg * private_c_reg_decay
            # noise *= (1 - nosie_decay)
    ...
    if episode % avg_span == 0 and episode > 0:
        ...
        print(
            ...
            (start_i, end_i,
             ...
             float(np.mean(voltage_off_limits[start_i: end_i]) / max_episode_length),
